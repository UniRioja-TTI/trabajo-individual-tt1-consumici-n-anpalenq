...
public class Solicitud {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("cantidadesIniciales", "nombreEntidades"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Solicitud.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in Solicitud is not found in the empty JSON string", Solicitud.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Solicitud.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `Solicitud` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("cantidadesIniciales") != null && !jsonObj.get("cantidadesIniciales").isJsonNull() && !jsonObj.get("cantidadesIniciales").isJsonArray()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `cantidadesIniciales` to be an array in the JSON string but got `%s`", jsonObj.get("cantidadesIniciales").toString()));
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("nombreEntidades") != null && !jsonObj.get("nombreEntidades").isJsonNull() && !jsonObj.get("nombreEntidades").isJsonArray()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `nombreEntidades` to be an array in the JSON string but got `%s`", jsonObj.get("nombreEntidades").toString()));
      }
  }
